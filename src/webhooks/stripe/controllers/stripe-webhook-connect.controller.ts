...
@Controller('webhook/stripe/connected')
export class StripeWebHookConnectController {
  constructor(
  ...
  async createStripeWebhookAction(@Req() req: RawBodyRequest<Request>) {
    ...
    try {
      event = this._stripeService.constructorEvent(
        req['rawBody'],
        signature,
        this._configService.getOrThrow(
          CONFIG_VAR.STRIPE_WEB_HOOK_SECRET_CONNECTED,
        ),
      );
